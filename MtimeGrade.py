...
for (k, v) in moviesdict.items():
    ...
    # 场次价格
    if ratint == -1:
        responseprice = requests.get("http://theater.mtime.com/China_Zhejiang_Province_Ningbo/movie/" + k + "/", header)
        pricesoup = BeautifulSoup(responseprice.text, "html.parser")
        print(pricesoup)
        for x in pricesoup.findAll('a', class_="timebox fl"):
            print(x)
